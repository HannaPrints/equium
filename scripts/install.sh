...
if [[ -z "${TMUX:-}" && -z "${STY:-}" && -t 1 ]]; then
  ...
  warn "   apt install -y tmux && tmux new -s eqm   # then $SELF"
fi
...
printf "${C_DIM}Press Ctrl-C to stop. Rerun $SELF to resume — RPC + keypair are saved.${C_RESET}\n"
if [[ -z "${EQUIUM_THREADS:-}" ]]; then
  printf "${C_DIM}Tip: EQUIUM_THREADS=32 $SELF   (boost CPU Wagner workers)${C_RESET}\n"
fi
hr
